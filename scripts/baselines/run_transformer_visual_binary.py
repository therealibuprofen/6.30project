...
from ultrasound_decoding.multiframe.factorized_transformer import (
    ...
    architecture_config,
    train_factorized_transformer_fold,
)
...
OUTPUT_VERSION = "transformer_visual_binary_v1"
TASK_NAME = "binary"
SEEDS = (0, 1, 2)
MAX_FOLDS = 10
FORMAL_EPOCHS = 40
...
REQUIRED_FINAL_OUTPUTS = (
    ...
    "transformer_per_seed.csv",
    "transformer_per_fold.csv",
    "transformer_predictions.csv",
    "transformer_confusion_matrices.csv",
    "transformer_training_history.csv",
    "transformer_vs_existing_baselines.csv",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Frozen clean4 CNN factorized Transformer within-session binary baseline."
    )
...
def frozen_experiment_config(batch_size: int) -> dict[str, Any]:
    architecture = FactorizedTransformerConfig()
    ...
    return {
        ...
        "baseline_claim": "lightweight factorized spatiotemporal Transformer baseline; not a proposed model",
        ...
        "oom_policy": "one uniform batch size for every session; only 16->8->4 permitted",
    }


def run_identity(project_root: Path, batch_size: int) -> dict[str, Any]:
    model_path = project_root / "src" / "ultrasound_decoding" / "multiframe" / "factorized_transformer.py"
...
def validate_completed_task(
    ...
    *,
    ...
    if fingerprint(model_config) != fingerprint(architecture_config(FactorizedTransformerConfig())):
...
def run_sanity(args: argparse.Namespace, identity: dict[str, Any]) -> None:
    ...
    model = CNNFactorizedTransformer(config).to(args.device)
    ...
    result = train_factorized_transformer_fold(
...
def write_fold_task(
    ...
    result = train_factorized_transformer_fold(
        ...
        architecture=FactorizedTransformerConfig(), device=args.device, workers=args.workers,
    )
...
def load_existing_baselines(args: argparse.Namespace) -> pd.DataFrame:
    selected_models = set(EXISTING_BASELINE_DISPLAY) - {MODEL_NAME, "sbind_noatt", "sbind"}
    rows: list[dict[str, Any]] = []
    ...
    combined = pd.concat([clean, sbind], ignore_index=True)
    expected_pairs = {
        (session, model)
        for session in EXPECTED_SESSIONS
...
def paired_comparison_rows(comparison: pd.DataFrame) -> pd.DataFrame:
    ...
    rows: list[dict[str, Any]] = []
    for baseline in ("cnn2d_meanpool", "sbind"):
...
def build_report(
    ...
    pivot = comparison.pivot(index="session", columns="model", values="mean_BA")
    transformer_mean = float(pivot[MODEL_NAME].mean())
    ...
    support = bool(
        cnn_stats["mean_delta_BA"] > 0
        and cnn_stats["improved_sessions"] >= 6
        and cnn_stats["exact_two_sided_sign_flip_p"] < 0.05
    )
    return "\n".join(
        [
            ...
            "",
            "本报告只解释冻结的 `cnn_factorized_transformer` baseline；没有据此修改模型或超参数。",
            "",
            f"1. Transformer 平均 BA={transformer_mean:.4f}，CNN mean-pool={cnn_mean:.4f}；"
            f"平均差={float(cnn_stats['mean_delta_BA']):+.4f}，exact p={float(cnn_stats['exact_two_sided_sign_flip_p']):.4f}。",
            f"2. SBIND-adapted 平均 BA={sbind_mean:.4f}；Transformer 相对 SBIND 平均差="
            f"{float(sbind_stats['mean_delta_BA']):+.4f}，exact p={float(sbind_stats['exact_two_sided_sign_flip_p']):.4f}。",
            f"3. 相对 CNN：改善/持平/下降={int(cnn_stats['improved_sessions'])}/"
            ...
            f"{int(sbind_stats['worsened_sessions'])}。",
            f"4. 强 session 708/709/710 的 Transformer BA 分别为 "
            + ", ".join(f"{session}:{float(strong_values.loc[session]):.3f}" for session in strong)
            + f"；达到或超过 0.90 的数量为 {int((strong_values >= 0.90).sum())}/3。",
            f"5. 按既有 CNN BA<=0.60 定义的弱 session 为 {weak}；其中相对 CNN 改善 "
            f"{int((weak_deltas > 1e-12).sum())}/{len(weak_deltas)}，平均差="
            f"{float(weak_deltas.mean()) if len(weak_deltas) else float('nan'):+.4f}。",
            f"6. 训练准确率>=0.95 且 OOF BA<=0.60 的 session 数为 {severe}/9。"
            "验证准确率不适用，因为冻结 benchmark 没有 validation/early stopping；正式选择始终为 epoch 40。",
            "7. " + (
                "结果满足预先定义的跨 session 一致性与配对检验条件，支持把长距离空间建模作为后续人工评估方向；"
                "仍不能把本 baseline 宣称为新方法。"
                if support else
                "结果未同时满足跨 session 一致改善和 exact 配对检验条件，因此尚不足以支持立即继续开发长距离空间建模方法。"
            ),
            "",
            "停止于 baseline 解释；不自动开发 Mamba 或 proposed model。",
        ]
    ) + "\n"
...
def aggregate_outputs(
    ...
    atomic_csv(args.output_dir / "transformer_per_fold.csv", per_fold)
    atomic_csv(args.output_dir / "transformer_predictions.csv", predictions)
    atomic_csv(args.output_dir / "transformer_confusion_matrices.csv", confusions)
    atomic_csv(args.output_dir / "transformer_training_history.csv", history)
    ...
    atomic_csv(args.output_dir / "transformer_per_seed.csv", per_seed)
    summary = (
    ...
    required_summary_columns = [
        ...
        "n_cycles", "n_samples", "parameter_count",
    ]
    summary = summary[required_summary_columns]
    atomic_csv(args.output_dir / "transformer_summary.csv", summary)

    existing = load_existing_baselines(args)
    transformer_rows = summary.assign(
        model_display=MODEL_DISPLAY_NAME,
        n_seeds=len(SEEDS),
        source=str(args.output_dir / "transformer_summary.csv"),
    )[
        ["session", "model", "model_display", "mean_BA", "std_BA", "mean_accuracy", "n_seeds", "source"]
    ]
    comparison = pd.concat([existing, transformer_rows], ignore_index=True).sort_values(
        ["session", "model"]
    )
    ...
    atomic_csv(args.output_dir / "transformer_vs_existing_baselines.csv", comparison)
    ...
    atomic_text(args.output_dir / "transformer_report.md", build_report(comparison, paired, overfitting))
