...
from ultrasound_decoding.multiframe.cyclemix_temporal1d import (
    CYCLEMIX_IMPLEMENTATION_VERSION,
    CYCLEMIX_VARIANT,
    INPUT_VARIANTS,
    LAMBDA_MIX,
...
OUTPUT_VERSION = "cyclemix_temporal1d_v1"
TASK_NAME = "binary"
SEEDS = (0, 1, 2)
MAX_FOLDS = 10
FORMAL_EPOCHS = 40
...
REQUIRED_FINAL_OUTPUTS = (
    ...
    "mix_coverage_audit.csv",
    "decision_rule_audit.json",
    "cyclemix_temporal1d_report.md",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paired formal clean4 raw vs training-CycleMix Temporal1D "
...
def source_paths(project_root: Path) -> list[Path]:
    return [
        ...
        project_root
        / "src/ultrasound_decoding/multiframe/cyclemix_temporal1d.py",
...
def frozen_experiment_config(batch_size: int) -> dict[str, Any]:
    return {
        ...
        "question": (
            "does training-only same-block cross-cycle 50:50 input mixup "
            "improve held-out-cycle generalization"
        ),
        ...
        "cyclemix": {
            "implementation_version": CYCLEMIX_IMPLEMENTATION_VERSION,
            "partner_rule": "same original block identity and different training cycle",
            "lambda_mix": LAMBDA_MIX,
            "lambda_searched": False,
            "partners_per_anchor_maximum": 1,
            "partner_pool": "current outer-training fold only",
            "real_samples_retained": True,
            "hard_binary_labels": True,
            "mixing_space": "after train-fold-only normalization",
        ...
        "preprocessing": {
            ...
            CYCLEMIX_VARIANT: (
                ...
                "training-only same-block different-cycle 50:50 mix"
            ),
        },
        ...
        "test_used_for_partner_pool": False,
        "test_mix_applied": False,
        "test_block_identity_used": False,
        "test_cycle_identity_input_to_model": False,
...
def validate_task_plan(plan: pd.DataFrame) -> dict[str, int]:
    ...
    if set(plan["variant"].astype(str)) != set(INPUT_VARIANTS):
        raise AssertionError("task plan variant coverage differs from raw + cyclemix")
    ...
    if not pair_counts.eq(len(INPUT_VARIANTS)).all():
        raise AssertionError("raw and cyclemix tasks are not perfectly paired")
...
def validate_completed_task(
    ...
    *,
    ...
    try:
        ...
        mix_audit = json.loads(
            (path / "mix_coverage_audit.json").read_text(encoding="utf-8")
        )
    ...
    if (
        str(mix_audit.get("session")),
        str(mix_audit.get("variant")),
        int(mix_audit.get("seed", -1)),
        int(mix_audit.get("fold", -1)),
    ) != identity:
        return fail("mix audit identity mismatch")
    if mix_audit.get("train_cycle_ids") != expected_train_cycles:
        return fail("mix partner-pool training-cycle provenance mismatch")
    if mix_audit.get("test_cycle_ids") != expected_test_cycles:
        return fail("mix test-cycle audit mismatch")
    if (
        bool(mix_audit.get("train_test_cycle_overlap", True))
        or mix_audit.get("partner_pool_scope")
        != "current_outer_training_samples_only"
        or bool(mix_audit.get("test_samples_in_partner_pool", True))
        or bool(mix_audit.get("test_block_identity_loaded_for_training", True))
        or bool(mix_audit.get("test_mix_applied", True))
    ):
        return fail("CycleMix training/test isolation audit failed")
    if not (
        bool(mix_audit.get("same_block_required", False))
        and bool(mix_audit.get("different_cycle_required", False))
        and bool(mix_audit.get("one_partner_per_anchor_maximum", False))
        and bool(mix_audit.get("real_samples_retained", False))
        and bool(mix_audit.get("hard_labels_used", False))
        and mix_audit.get("mixing_space") == "after_train_fold_normalization"
        and np.isclose(float(mix_audit.get("lambda_mix", np.nan)), LAMBDA_MIX)
    ):
        return fail("frozen CycleMix definition mismatch")
    for field in (
        "same_cycle_violation_count",
        "different_block_violation_count",
        "self_pair_violation_count",
    ):
        if int(mix_audit.get(field, -1)) != 0:
            return fail(f"nonzero {field}")
    total_real = int(mix_audit.get("number_of_real_training_samples", -1))
    total_mixed = int(mix_audit.get("number_of_mixed_samples_generated", -1))
    coverage = float(mix_audit.get("mix_coverage", np.nan))
    if total_real <= 0 or total_mixed < 0 or not np.isclose(
        coverage, total_mixed / total_real, atol=1e-12
    ):
        return fail("invalid mix coverage totals")
    if variant == RAW_VARIANT and (total_mixed != 0 or coverage != 0.0):
        return fail("raw task unexpectedly generated mixed samples")
    if variant == CYCLEMIX_VARIANT and not (0.0 <= coverage <= 1.0):
        return fail("CycleMix coverage lies outside [0,1]")
    if (
        bool(normalization.get("synthetic_samples_used_for_normalization_fit", True))
        or not bool(normalization.get("normalization_fit_before_cyclemix", False))
        or bool(normalization.get("test_mix_applied", True))
        or bool(normalization.get("test_block_identity_used", True))
    ):
        return fail("normalization/mixing order audit failed")
    if (
    ...
    history_required = {
        ...
        "train_loss",
        "train_accuracy",
        "final_train_accuracy_real",
        "mean_training_loss",
        "number_of_real_training_samples",
        "number_of_mixed_samples_generated",
        "mix_coverage",
        "number_of_anchors_without_valid_partner",
        "same_cycle_violation_count",
        "different_block_violation_count",
        "self_pair_violation_count",
    }
    ...
    history_real = int(history["number_of_real_training_samples"].sum())
    history_mixed = int(history["number_of_mixed_samples_generated"].sum())
    if history_real != total_real or history_mixed != total_mixed:
        return fail("mix audit totals differ from epoch history")
    if int(result.get("total_mixed_samples", -1)) != total_mixed:
        return fail("task result mixed-sample total differs from audit")
    if not np.isclose(
        float(result.get("mean_mix_coverage", np.nan)),
        float(history["mix_coverage"].mean()),
        atol=1e-12,
    ):
        return fail("task result mean mix coverage differs from history")
    for field in (
        "same_cycle_violation_count",
        "different_block_violation_count",
        "self_pair_violation_count",
    ):
        if int(history[field].sum()) != 0:
            return fail(f"training history contains nonzero {field}")
    ...
    if not np.isclose(
        ...
        float(history.iloc[-1]["final_train_accuracy_real"]),
        atol=1e-12,
    ):
        return fail("stored final_train_accuracy differs from final real-sample accuracy")
    if not np.isclose(
        float(result.get("final_train_accuracy_real", np.nan)),
        float(history.iloc[-1]["final_train_accuracy_real"]),
        atol=1e-12,
    ):
        return fail("stored final_train_accuracy_real differs from history")
    if not np.isclose(
...
def run_sanity(args: argparse.Namespace, identity: dict[str, Any]) -> None:
        ...
        audits.append(
            {
                ...
                "number_of_real_training_samples": trained.mix_audit[
                    "number_of_real_training_samples"
                ],
                "number_of_mixed_samples_generated": trained.mix_audit[
                    "number_of_mixed_samples_generated"
                ],
                "mix_coverage": trained.mix_audit["mix_coverage"],
                "anchors_without_partner": trained.mix_audit[
                    "number_of_anchors_without_valid_partner"
                ],
                "same_cycle_violation_count": trained.mix_audit[
                    "same_cycle_violation_count"
                ],
                "different_block_violation_count": trained.mix_audit[
                    "different_block_violation_count"
                ],
                "partner_examples": trained.mix_audit["partner_examples"],
    ...
    framework.atomic_json(
        args.output_dir / "sanity" / "sanity_audit.json",
        {
            ...
            "cycle_overlap": False,
            "preprocessing_order": (
                "clean4 -> arcsinh -> train-fold-only pixel z-score -> "
                "training-only CycleMix"
            ),
            "test_metadata_used_for_inference": False,
...
def write_fold_task(
    ...
    final_train_accuracy = float(history.iloc[-1]["final_train_accuracy_real"])
    ...
    result = {
        **shared,
        ...
        "final_train_accuracy_real": final_train_accuracy,
        ...
        "mean_mix_coverage": float(history["mix_coverage"].mean()),
        "total_mixed_samples": int(
            trained.mix_audit["number_of_mixed_samples_generated"]
        ),
        "anchors_without_partner": int(
            trained.mix_audit["number_of_anchors_without_valid_partner"]
        ),
        "same_cycle_violation_count": int(
            trained.mix_audit["same_cycle_violation_count"]
        ),
        "different_block_violation_count": int(
            trained.mix_audit["different_block_violation_count"]
        ),
    }
    ...
    framework.atomic_json(path / "mix_coverage_audit.json", trained.mix_audit)
...
def read_all_tasks(args: argparse.Namespace, plan: pd.DataFrame, run_fp: str):
    results, predictions, confusions, histories, mix_audits = [], [], [], [], []
    for expected in plan.to_dict(orient="records"):
        path = task_dir(
            args.output_dir,
            str(expected["session"]),
        ...
        mix_audits.append(
            {
                **mix_audit,
                "train_cycle_ids": framework.canonical_json(
                    mix_audit["train_cycle_ids"]
                ),
                "test_cycle_ids": framework.canonical_json(
                    mix_audit["test_cycle_ids"]
                ),
                "partner_examples": framework.canonical_json(
                    mix_audit["partner_examples"]
                ),
            }
        )
    return (
        ...
        rows.append(
            {
                ...
                "mean_mix_coverage": float(
                    fold_source["mean_mix_coverage"].astype(float).mean()
                ),
...
def build_session_summary(seed_summary: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        ...
        .agg(
            ...
            mix_coverage=("mean_mix_coverage", "mean"),
        )
    ...
    mix_coverage = grouped.pivot(
        index="session", columns="variant", values="mix_coverage"
    )
    rows = []
    for session in EXPECTED_SESSIONS:
        rows.append(
            {
                ...
                "cyclemix_BA": float(ba.loc[session, CYCLEMIX_VARIANT]),
                "delta_BA": float(
                    ba.loc[session, CYCLEMIX_VARIANT]
                    - ba.loc[session, RAW_VARIANT]
                ),
                "raw_train_acc": float(train.loc[session, RAW_VARIANT]),
                "cyclemix_train_acc": float(train.loc[session, CYCLEMIX_VARIANT]),
                "raw_gap": float(gap.loc[session, RAW_VARIANT]),
                "cyclemix_gap": float(gap.loc[session, CYCLEMIX_VARIANT]),
                "delta_gap": float(
                    gap.loc[session, CYCLEMIX_VARIANT]
                    - gap.loc[session, RAW_VARIANT]
                ),
                "mix_coverage": float(
                    mix_coverage.loc[session, CYCLEMIX_VARIANT]
                ),
            }
        )
...
def build_overall_and_decision(
    ...
    cyclemix_mean = float(session_summary["cyclemix_BA"].mean())
    overall_delta = float(deltas.mean())
    strong_raw = float(strong["raw_BA"].mean())
    strong_cyclemix = float(strong["cyclemix_BA"].mean())
    weak_raw = float(weak["raw_BA"].mean())
    weak_cyclemix = float(weak["cyclemix_BA"].mean())
        ...
        [
            {
                ...
                "cyclemix_mean_BA": cyclemix_mean,
                "overall_delta_BA": overall_delta,
                "median_delta_BA": float(deltas.median()),
                "raw_strong_mean_BA": strong_raw,
                "cyclemix_strong_mean_BA": strong_cyclemix,
                "strong_delta_BA": strong_cyclemix - strong_raw,
                "raw_weak_mean_BA": weak_raw,
                "cyclemix_weak_mean_BA": weak_cyclemix,
                "weak_delta_BA": weak_cyclemix - weak_raw,
    ...
    decision = {
        ...
        "decision": (
            "supports_continue_cyclemix_route"
            if all(checks.values()) and not_single_session_driven
            else "does_not_support_cyclemix_route"
        ),
...
def build_report(
    ...
    test = paired.iloc[0]
    lines = [
        "# CycleMix Temporal1D v1",
        "",
        "## Frozen protocol",
        "",
        "Raw and CycleMix are both rerun. They share the exact clean4 samples, "
        ...
        "- cyclemix: `clean4 -> arcsinh -> outer-train-fold pixel z-score -> training-only 50:50 same-block different-cycle mix -> Temporal1D`",
        "",
        "For every real training anchor, CycleMix samples at most one partner from the "
        "same original block identity and a different cycle in the current outer-training "
        "fold. The mixed input is exactly `0.5*X_anchor_norm + 0.5*X_partner_norm` "
        "with the same hard binary label. Real and mixed inputs are concatenated and one "
        "mean cross-entropy is computed. Test samples are never mixed; test block/cycle "
        "metadata are not model inputs. Final train accuracy is fixed epoch 40 on real "
        "samples only. The baseline has no early stopping.",
        "",
        "Historical raw baseline reused: **no**.",
        "",
        "## Session results",
        "",
        "| session | raw train | raw BA | raw gap | cyclemix train | cyclemix BA | cyclemix gap | delta BA | delta gap | mix coverage |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in session_summary.itertuples(index=False):
        lines.append(
            f"| {row.session} | {row.raw_train_acc:.4f} | {row.raw_BA:.4f} | "
            f"{row.raw_gap:.4f} | {row.cyclemix_train_acc:.4f} | "
            f"{row.cyclemix_BA:.4f} | {row.cyclemix_gap:.4f} | "
            f"{row.delta_BA:+.4f} | {row.delta_gap:+.4f} | "
            f"{row.mix_coverage:.4f} |"
        )
    lines.extend(
        [
            "",
            ...
            f"- CycleMix mean BA: {summary.cyclemix_mean_BA:.4f}",
            f"- Overall delta BA: {summary.overall_delta_BA:+.4f}",
            f"- Median session delta BA: {summary.median_delta_BA:+.4f}",
            f"- Strong-3 raw/cyclemix/delta: {summary.raw_strong_mean_BA:.4f} / "
            f"{summary.cyclemix_strong_mean_BA:.4f} / {summary.strong_delta_BA:+.4f}",
            f"- Weak-6 raw/cyclemix/delta: {summary.raw_weak_mean_BA:.4f} / "
            f"{summary.cyclemix_weak_mean_BA:.4f} / {summary.weak_delta_BA:+.4f}",
...
def aggregate_outputs(
    ...
    run_fp = framework.fingerprint(identity)
    per_fold, predictions, confusions, history, mix_audits = read_all_tasks(
        args, plan, run_fp
    )
    ...
    if len(mix_audits) != len(plan):
        raise AssertionError("mix-coverage task-audit coverage is incomplete")
    if not mix_audits[
        [
            "same_cycle_violation_count",
            "different_block_violation_count",
        ...
        raise AssertionError("illegal CycleMix pairs detected during aggregation")
    ...
    framework.atomic_csv(args.output_dir / "mix_coverage_audit.csv", mix_audits)
    ...
    framework.atomic_text(
        args.output_dir / "cyclemix_temporal1d_report.md",
...
def run_cuda_preflight(args: argparse.Namespace) -> None:
    ...
    for variant in INPUT_VARIANTS:
        ...
        forward_batch = (
            2 * FORMAL_BATCH_SIZE
            if variant == CYCLEMIX_VARIANT
            else FORMAL_BATCH_SIZE
        )
        inputs = torch.zeros((forward_batch, 4, 1, 128, 501), device=device)
        targets = torch.arange(forward_batch, device=device) % 2
        ...
        audits.append(
            {
                ...
                "configured_real_batch_size": FORMAL_BATCH_SIZE,
    ...
    framework.atomic_json(
        args.output_dir / "audit" / "cuda_cyclemix_memory_preflight.json",
