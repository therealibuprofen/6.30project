...
OUTPUT_VERSION = "spatial_demean_temporal1d_v1"
TASK_NAME = "binary"
SEEDS = (0, 1, 2)
MAX_FOLDS = 10
FORMAL_EPOCHS = 40
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paired formal clean4 raw vs per-frame spatial-demean Temporal1D "
...
def source_paths(project_root: Path) -> list[Path]:
    return [
        ...
        project_root
        / "src/ultrasound_decoding/multiframe/spatial_demean_temporal1d.py",
...
def frozen_experiment_config(batch_size: int) -> dict[str, Any]:
    return {
        ...
        "question": "does frame minus its own spatial mean improve generalization",
        ...
        "spatial_demean_formula": "X[t,h,w] - mean_h,w(X[t,h,w])",
        "preprocessing": {
...
def validate_task_plan(plan: pd.DataFrame) -> dict[str, int]:
    ...
    if set(plan["variant"].astype(str)) != set(INPUT_VARIANTS):
        raise AssertionError("task plan variant coverage differs from raw + spatial_demean")
    ...
    if not pair_counts.eq(len(INPUT_VARIANTS)).all():
        raise AssertionError("raw and spatial_demean tasks are not perfectly paired")
...
def validate_completed_task(
    ...
    *,
    ...
    if variant == SPATIAL_DEMEAN_VARIANT:
        if normalization.get("spatial_demean_statistics_scope") != "one_sample_one_frame_only":
            return fail("spatial demean scope mismatch")
        if not bool(normalization.get("spatial_demean_applied", False)):
            return fail("spatial demean marker missing")
    elif bool(normalization.get("spatial_demean_applied", True)):
        return fail("raw task unexpectedly applied spatial demean")
    if (
    ...
    if not np.isclose(
        ...
        return fail("stored train_accuracy differs from history")
    if not np.isclose(
...
def run_sanity(args: argparse.Namespace, identity: dict[str, Any]) -> None:
    ...
    tiny_train = framework.select_balanced_indices(data.y, train_idx, per_class=2)
    tiny_test = framework.select_balanced_indices(data.y, test_idx, per_class=1)
    device = resolve_device(args.device)
    audits = []
    for variant in INPUT_VARIANTS:
        transformed = np.arcsinh(data.X[tiny_train[:2]].astype(np.float32))
        if variant == SPATIAL_DEMEAN_VARIANT:
            transformed = spatial_demean_per_frame(transformed)
            if not np.allclose(transformed.mean(axis=(-2, -1)), 0.0, atol=2e-6):
                raise AssertionError("spatial demean sanity means are not zero")
        ...
        rows.append(
            {
                ...
                "spatial_demean_BA": float(ba.loc[session, SPATIAL_DEMEAN_VARIANT]),
                "delta_BA": float(
                    ba.loc[session, SPATIAL_DEMEAN_VARIANT]
                    - ba.loc[session, RAW_VARIANT]
                ),
                "raw_train_acc": float(train.loc[session, RAW_VARIANT]),
                "demean_train_acc": float(train.loc[session, SPATIAL_DEMEAN_VARIANT]),
                "raw_gap": float(gap.loc[session, RAW_VARIANT]),
                "demean_gap": float(gap.loc[session, SPATIAL_DEMEAN_VARIANT]),
                "delta_gap": float(
                    gap.loc[session, SPATIAL_DEMEAN_VARIANT]
...
def build_overall_and_decision(
    ...
    demean_mean = float(session_summary["spatial_demean_BA"].mean())
    overall_delta = float(deltas.mean())
    strong_raw = float(strong["raw_BA"].mean())
    strong_demean = float(strong["spatial_demean_BA"].mean())
    weak_raw = float(weak["raw_BA"].mean())
    weak_demean = float(weak["spatial_demean_BA"].mean())
        ...
        [
            {
                ...
                "spatial_demean_mean_BA": demean_mean,
                "overall_delta_BA": overall_delta,
                "raw_strong_mean_BA": strong_raw,
                "demean_strong_mean_BA": strong_demean,
                "strong_delta_BA": strong_demean - strong_raw,
                "raw_weak_mean_BA": weak_raw,
                "demean_weak_mean_BA": weak_demean,
                "weak_delta_BA": weak_demean - weak_raw,
    ...
    paired = pd.DataFrame(
        [
            {
                "comparison": "spatial_demean_vs_raw",
    ...
    decision = {
        ...
        "decision": (
            "supports_continue_spatial_pattern_route"
            if all(checks.values()) and not_single_session_driven
            else "does_not_support_spatial_demean_route"
        ),
...
def build_report(
    ...
    lines = [
        ...
        "## Frozen protocol",
        "",
        "Raw and spatial-demean are both rerun. They share the exact clean4 samples, "
        ...
        "- spatial_demean: `clean4 -> arcsinh -> each frame minus its H,W mean -> outer-train-fold pixel z-score -> Temporal1D`",
        "",
        "The per-frame demean is sample-wise and deterministic. Test samples are demeaned "
        "only with their own frame means. Test-fold data do not fit normalization or "
        "select epochs/models. The formal baseline has no early stopping; all tasks train "
        "for exactly 40 epochs.",
        "",
        "Historical raw baseline reused: **no**.",
        "",
        "## Session results",
        "",
        "| session | raw train | raw BA | raw gap | demean train | demean BA | demean gap | delta BA | delta gap |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in session_summary.itertuples(index=False):
        lines.append(
            f"| {row.session} | {row.raw_train_acc:.4f} | {row.raw_BA:.4f} | "
            f"{row.raw_gap:.4f} | {row.demean_train_acc:.4f} | "
            f"{row.spatial_demean_BA:.4f} | {row.demean_gap:.4f} | "
    ...
    lines.extend(
        [
            ...
            f"- Spatial-demean mean BA: {summary.spatial_demean_mean_BA:.4f}",
            f"- Overall delta BA: {summary.overall_delta_BA:+.4f}",
            f"- Strong-3 raw/demean/delta: {summary.raw_strong_mean_BA:.4f} / "
            f"{summary.demean_strong_mean_BA:.4f} / {summary.strong_delta_BA:+.4f}",
            f"- Weak-6 raw/demean/delta: {summary.raw_weak_mean_BA:.4f} / "
            f"{summary.demean_weak_mean_BA:.4f} / {summary.weak_delta_BA:+.4f}",
...
def aggregate_outputs(
    ...
    framework.atomic_text(
        args.output_dir / "spatial_demean_temporal1d_report.md",
