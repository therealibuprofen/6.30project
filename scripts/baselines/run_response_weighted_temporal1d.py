...
from ultrasound_decoding.multiframe.response_weighted_temporal1d import (
    ...
    RAW_VARIANT,
    RESPONSE_MAP_IMPLEMENTATION_VERSION,
...
OUTPUT_VERSION = "response_weighted_temporal1d_v1"
TASK_NAME = "binary"
SEEDS = (0, 1, 2)
MAX_FOLDS = 10
FORMAL_EPOCHS = 40
...
REQUIRED_FINAL_OUTPUTS = (
    ...
    "response_map_audit.csv",
    "decision_rule_audit.json",
    "response_weighted_temporal1d_report.md",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paired formal clean4 raw vs training-response-weighted Temporal1D "
...
def source_paths(project_root: Path) -> list[Path]:
    return [
        ...
        project_root
        / "src/ultrasound_decoding/multiframe/response_weighted_temporal1d.py",
...
def frozen_experiment_config(batch_size: int) -> dict[str, Any]:
    return {
        ...
        "question": "does a training-cycle-only soft response map improve generalization",
        ...
        "response_map": {
            "implementation_version": RESPONSE_MAP_IMPLEMENTATION_VERSION,
            "construction_scope": "outer-training cycles only",
            "block_images": "arcsinh(clean4).mean(axis=time)",
            "cycle_contrast": "0.5*(G + D) - 0.5*(S + T)",
            "score": "abs(mean_cycle(C)) / (std_cycle(C, ddof=0) + 1e-6)",
            "percentile_rank": "average ranks; P=(rank-1)/(n_pixels-1)",
            "weight": "W=0.5+P, bounded [0.5,1.5], spatial mean 1.0",
            "cache_scope": "session + fold + train cycles + version + source hash",
            "test_data_or_statistics_used": False,
        },
        "preprocessing": {
            RAW_VARIANT: "clean4 -> arcsinh -> train_fold_pixel_zscore",
            RESPONSE_WEIGHTED_VARIANT: (
                "clean4 -> arcsinh -> train_fold_pixel_zscore -> "
                "multiply same training-derived W into train and test"
            ),
        },
...
def validate_task_plan(plan: pd.DataFrame) -> dict[str, int]:
    ...
    if set(plan["variant"].astype(str)) != set(INPUT_VARIANTS):
        raise AssertionError("task plan variant coverage differs from raw + response_weighted")
    ...
    if not pair_counts.eq(len(INPUT_VARIANTS)).all():
        raise AssertionError("raw and response_weighted tasks are not perfectly paired")
    ...
    *,
    ...
    try:
        ...
        map_audit = json.loads(
            (path / "response_map_audit.json").read_text(encoding="utf-8")
        )
    ...
    if variant == RESPONSE_WEIGHTED_VARIANT:
        ...
        if not bool(normalization.get("response_weighting_after_normalization", False)):
            return fail("response weighting was not applied after normalization")
        if normalization.get("response_map_fit_scope") != "training_cycles_only":
            return fail("response map was not fit on training cycles only")
        if bool(normalization.get("test_data_used_to_fit_response_map", True)):
            return fail("test data used to fit response map")
        if map_audit.get("applicable") is not True:
            return fail("weighted task lacks applicable response-map audit")
        if (
            str(map_audit.get("session")),
            int(map_audit.get("fold", -1)),
        ) != (session, fold):
            return fail("response-map identity mismatch")
        if map_audit.get("train_cycle_ids") != expected_train_cycles:
            return fail("response-map training-cycle provenance mismatch")
        if map_audit.get("test_cycle_ids") != expected_test_cycles:
            return fail("response-map test-cycle audit mismatch")
        if bool(map_audit.get("train_test_cycle_overlap", True)):
            return fail("response-map train/test cycle overlap")
        if (
            map_audit.get("construction_scope") != "training_cycles_only"
            or bool(map_audit.get("full_session_glm_loaded", True))
            or map_audit.get("response_map_shape") != [128, 501]
        ):
            return fail("response-map construction scope or shape mismatch")
        if not (
            0.5 - 1e-6 <= float(map_audit.get("weight_map_min", np.nan))
            <= float(map_audit.get("weight_map_max", np.nan)) <= 1.5 + 1e-6
            and np.isclose(float(map_audit.get("weight_map_mean", np.nan)), 1.0, atol=2e-6)
        ):
            return fail("response-map weights violate frozen range/mean")
        if (
            result.get("response_map_hash") != map_audit.get("response_map_hash")
            or result.get("weight_map_hash") != map_audit.get("weight_map_hash")
            or normalization.get("response_map_hash") != map_audit.get("response_map_hash")
            or normalization.get("weight_map_hash") != map_audit.get("weight_map_hash")
            or normalization.get("train_and_test_weight_map_hash")
            != map_audit.get("weight_map_hash")
        ):
            return fail("response-map hash mismatch across artifacts")
        if result.get("response_map_cache_key") != map_audit.get(
            "response_map_cache_key"
        ):
            return fail("response-map cache-key mismatch")
    else:
        if bool(normalization.get("response_weighting_after_normalization", True)):
            return fail("raw task unexpectedly applied response weighting")
        if map_audit.get("applicable") is not False:
            return fail("raw task has an applicable response map")
        if result.get("response_map_hash") is not None:
            return fail("raw task unexpectedly stores a response-map hash")
    if (
...
def response_map_for_fold(
    ...
    *,
    session: str,
    fold: int,
    cache: dict[str, TrainingResponseMap],
) -> tuple[TrainingResponseMap, dict[str, Any]]:
    train_cycles = tuple(sorted(int(value) for value in np.unique(data.groups[train_idx])))
    test_cycles = tuple(sorted(int(value) for value in np.unique(data.groups[test_idx])))
    source_key = "src/ultrasound_decoding/multiframe/response_weighted_temporal1d.py"
    cache_key = response_map_cache_key(
        session=session,
        fold=fold,
        train_cycle_ids=train_cycles,
        relevant_source_sha256=identity["project_source_sha256"][source_key],
    )
    if cache_key not in cache:
        cache[cache_key] = build_training_response_map(
            data.X[train_idx],
            data.metadata.iloc[train_idx]["block_name"].astype(str).to_numpy(),
            data.groups[train_idx],
        )
    response_map = cache[cache_key]
    audit = response_map_audit(
        response_map,
        session=session,
        fold=fold,
        train_cycle_ids=train_cycles,
        test_cycle_ids=test_cycles,
        cache_key=cache_key,
    )
    audit["applicable"] = True
    shared_dir = args.output_dir / "response_maps" / f"session_{session}" / f"fold_{fold:02d}"
    framework.atomic_json(shared_dir / "response_map_audit.json", audit)
    if fold == 1:
        shared_dir.mkdir(parents=True, exist_ok=True)
        np.savez_compressed(
            shared_dir / "response_score_and_weight.npz",
            response_score=response_map.response_score,
            weight_map=response_map.weight_map,
            train_cycle_ids=np.asarray(response_map.train_cycle_ids, dtype=np.int64),
        )
...
def run_sanity(args: argparse.Namespace, identity: dict[str, Any]) -> None:
    ...
    response_map = build_training_response_map(
        data.X[tiny_train],
        data.metadata.iloc[tiny_train]["block_name"].astype(str).to_numpy(),
        data.groups[tiny_train],
    )
    sanity_cache_key = response_map_cache_key(
        session="710",
        fold=1,
        train_cycle_ids=selected_train_cycles,
        relevant_source_sha256=identity["project_source_sha256"][
            "src/ultrasound_decoding/multiframe/response_weighted_temporal1d.py"
        ],
    )
    map_audit = response_map_audit(
        response_map,
        session="710",
        fold=1,
        train_cycle_ids=selected_train_cycles,
        test_cycle_ids=selected_test_cycles,
        cache_key=sanity_cache_key,
    )
    ...
    for variant in INPUT_VARIANTS:
        ...
        trained = train_fold(
            ...
            response_map=(response_map if variant == RESPONSE_WEIGHTED_VARIANT else None),
            session="710",
            fold=1,
            seed=0,
        ...
        audits.append(
            {
                ...
                "response_map_hash": (
                    response_map.response_map_hash
                    if variant == RESPONSE_WEIGHTED_VARIANT
                    else None
                ),
...
def write_fold_task(
    ...
    result = {
        **shared,
        ...
        "response_map_hash": (
            response_map.response_map_hash if response_map is not None else None
        ),
        "response_map_id": map_audit.get("response_map_cache_key"),
        "weight_map_hash": (
            response_map.weight_map_hash if response_map is not None else None
        ),
        "response_map_cache_key": map_audit.get("response_map_cache_key"),
    }
    ...
    framework.atomic_json(path / "response_map_audit.json", map_audit)
...
def read_all_tasks(args: argparse.Namespace, plan: pd.DataFrame, run_fp: str):
    results, predictions, confusions, histories, response_audits = [], [], [], [], []
    for expected in plan.to_dict(orient="records"):
        path = task_dir(
            args.output_dir,
            str(expected["session"]),
        ...
        rows.append(
            {
                ...
                "response_weighted_BA": float(ba.loc[session, RESPONSE_WEIGHTED_VARIANT]),
                "delta_BA": float(
                    ba.loc[session, RESPONSE_WEIGHTED_VARIANT]
                    - ba.loc[session, RAW_VARIANT]
                ),
                "raw_train_acc": float(train.loc[session, RAW_VARIANT]),
                "weighted_train_acc": float(train.loc[session, RESPONSE_WEIGHTED_VARIANT]),
                "raw_gap": float(gap.loc[session, RAW_VARIANT]),
                "weighted_gap": float(gap.loc[session, RESPONSE_WEIGHTED_VARIANT]),
                "delta_gap": float(
                    gap.loc[session, RESPONSE_WEIGHTED_VARIANT]
...
def build_overall_and_decision(
    ...
    weighted_mean = float(session_summary["response_weighted_BA"].mean())
    overall_delta = float(deltas.mean())
    strong_raw = float(strong["raw_BA"].mean())
    strong_weighted = float(strong["response_weighted_BA"].mean())
    weak_raw = float(weak["raw_BA"].mean())
    weak_weighted = float(weak["response_weighted_BA"].mean())
        ...
        [
            {
                ...
                "response_weighted_mean_BA": weighted_mean,
                "overall_delta_BA": overall_delta,
                "median_delta_BA": float(deltas.median()),
                "raw_strong_mean_BA": strong_raw,
                "weighted_strong_mean_BA": strong_weighted,
                "strong_delta_BA": strong_weighted - strong_raw,
                "raw_weak_mean_BA": weak_raw,
                "weighted_weak_mean_BA": weak_weighted,
                "weak_delta_BA": weak_weighted - weak_raw,
    ...
    paired = pd.DataFrame(
        [
            {
                "comparison": "response_weighted_vs_raw",
    ...
    decision = {
        ...
        "decision": (
            "supports_continue_response_guided_route"
            if all(checks.values()) and not_single_session_driven
            else "does_not_support_response_guided_route"
        ),
...
def build_report(
    ...
    lines = [
        ...
        "## Frozen protocol",
        "",
        "Raw and response-weighted are both rerun. They share the exact clean4 samples, "
        ...
        "- response_weighted: `clean4 -> arcsinh -> outer-train-fold pixel z-score -> multiply fixed training-derived W -> Temporal1D`",
        "",
        "For each outer fold, W is constructed only from its training cycles. Each block "
        "is averaged over clean4 after arcsinh; each training cycle contributes "
        "`0.5*(G+D)-0.5*(S+T)`. The absolute across-cycle mean divided by population "
        "standard deviation plus 1e-6 is average-tie percentile ranked, and W=0.5+P. "
        "The same W is multiplied into normalized training and test blocks. No held-out "
        "sample, statistic, full-session GLM, hard mask, top-k rule, or ROI is used. "
        "The formal baseline has no early stopping; all tasks train for exactly 40 epochs.",
        "",
        "Historical raw baseline reused: **no**.",
        "",
        "## Session results",
        "",
        "| session | raw train | raw BA | raw gap | weighted train | weighted BA | weighted gap | delta BA | delta gap |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in session_summary.itertuples(index=False):
        lines.append(
            f"| {row.session} | {row.raw_train_acc:.4f} | {row.raw_BA:.4f} | "
            f"{row.raw_gap:.4f} | {row.weighted_train_acc:.4f} | "
            f"{row.response_weighted_BA:.4f} | {row.weighted_gap:.4f} | "
    ...
    lines.extend(
        [
            ...
            f"- Response-weighted mean BA: {summary.response_weighted_mean_BA:.4f}",
            f"- Overall delta BA: {summary.overall_delta_BA:+.4f}",
            f"- Median session delta BA: {summary.median_delta_BA:+.4f}",
            f"- Strong-3 raw/weighted/delta: {summary.raw_strong_mean_BA:.4f} / "
            f"{summary.weighted_strong_mean_BA:.4f} / {summary.strong_delta_BA:+.4f}",
            f"- Weak-6 raw/weighted/delta: {summary.raw_weak_mean_BA:.4f} / "
            f"{summary.weighted_weak_mean_BA:.4f} / {summary.weak_delta_BA:+.4f}",
...
def aggregate_outputs(
    ...
    run_fp = framework.fingerprint(identity)
    per_fold, predictions, confusions, history, response_audits = read_all_tasks(
        args, plan, run_fp
    )
    ...
    # Session fold counts are 9 or 10, so derive coverage from the frozen plan.
    expected_weighted = int(plan["variant"].eq(RESPONSE_WEIGHTED_VARIANT).sum())
    if len(response_audits) != expected_weighted:
        raise AssertionError("response-map task-audit coverage is incomplete")
    audit_groups = response_audits.groupby(["session", "fold"], sort=True)
    if not audit_groups["response_map_hash"].nunique().eq(1).all():
        raise AssertionError("seeds within a fold did not reuse the same response map")
    if not audit_groups["weight_map_hash"].nunique().eq(1).all():
        raise AssertionError("seeds within a fold did not reuse the same response weight")
    fold_response_audits = (
        response_audits.sort_values(["session", "fold", "seed"])
        .drop_duplicates(["session", "fold"])
        .drop(columns=["seed"])
    )
    ...
    framework.atomic_csv(args.output_dir / "response_map_audit.csv", fold_response_audits)
    ...
    framework.atomic_text(
        args.output_dir / "response_weighted_temporal1d_report.md",
...
def run_full(args: argparse.Namespace, identity: dict[str, Any]) -> None:
    ...
    for session in map(str, args.sessions):
        data, splits = audit_session(args, session)
        response_cache: dict[str, TrainingResponseMap] = {}
        ...
        for expected in session_plan.to_dict(orient="records"):
            ...
            if variant == RESPONSE_WEIGHTED_VARIANT:
                response_map, map_audit = response_map_for_fold(
                    args,
                    identity,
                    data,
                    train_idx,
                ...
                map_audit = {
                    ...
                    "weight_map_hash": None,
                    "response_map_cache_key": None,
                }
            path = task_dir(
                args.output_dir,
                session,
